...
- [x] **Phase B.2** — In-game **custom menu** + **Build Mode** (Aracdia's per-player creative): the new `aracdia_menu` mod adds a `/menu` chat command and an Aux1+Sneak shortcut that pop a custom formspec (Reprendre · Mode construction · Menu Luanti · Quitter). Build Mode grants fly / fast / noclip / invincibility / instabreak and unlocks a paginated creative inventory with full-text search; placed nodes are refunded so blocks are effectively infinite. The state is persisted per-player in the world.
- [x] **Phase B.3** — **Engine-level Esc interception**. The `aracdia-engine` fork [v0.2.0](https://github.com/Urac-coder/aracdia-engine/releases/tag/v0.2.0) introduces a new `TOSERVER_PAUSE_MENU` packet (protocol bump 52→53), a server-side `core.register_on_pause_menu(function(player) end)` Lua hook and a `TOCLIENT_SHOW_NATIVE_PAUSE_MENU` fallback. Pressing Esc in-game now runs the registered Aracdia handler (which opens our menu); if the server has none registered, the client opens the original Luanti pause menu — vanilla behaviour preserved. Cross-version compat is guarded on both sides: a patched client on a vanilla server (≤52) opens the native menu locally instead of sending an unknown opcode.
