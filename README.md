...
- [x] **Phase B.2** — In-game **custom menu** + **Build Mode** (Aracdia's per-player creative): the new `aracdia_menu` mod adds a `/menu` chat command and an Aux1+Sneak shortcut that pop a custom formspec (Reprendre · Mode construction · Menu Luanti · Quitter). Build Mode grants fly / fast / noclip / invincibility / instabreak and unlocks a paginated creative inventory with full-text search; placed nodes are refunded so blocks are effectively infinite. The state is persisted per-player in the world. Esc still opens the Luanti vanilla menu (engine-level binding) — that hook moves to Phase B.3 and requires patching the engine fork.
